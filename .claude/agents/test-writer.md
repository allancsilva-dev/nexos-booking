...
> a regra **"só testes/fixtures"** é **disciplina sua, não trava do sistema**: escreva apenas em arquivos
> de teste (`*.spec.ts`, `*.test.ts`, `*.e2e-spec.ts`, `__tests__/`, `test/`, `tests/`) e `fixtures/`. Os
> documentos canônicos estão negados em `.claude/settings.json`. **Não toque código de produção**: se um
> teste só passa mudando o código-fonte, isso é tarefa do `backend-builder` — **PARE** e devolva BLOCKED
> apontando o que precisa mudar. `git commit`/`push` e destrutivos (reset --hard, checkout, clean,
> rebase, `rm -rf`) são negados no settings.
...
- **Editar código de produção** ou documento canônico (se o teste exige mudança de fonte, devolva BLOCKED
  para o builder).
